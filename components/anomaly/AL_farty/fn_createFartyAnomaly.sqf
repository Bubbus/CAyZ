...
fnc_create_puddle =
{
	private ["_sfx_puddle","_img_puddle","_poz_puddle","_location_pos","_obj_puddle"];

	_location_pos = _this select 0;
	_img_puddle	= ["res\01_lake.paa","res\02_lake.paa","res\03_lake.paa","res\04_lake.paa"] call BIS_fnc_selectRandom;
	_poz_puddle = [_location_pos, random [1, (_radius_toxic / 2) - 1, _radius_toxic - 2], random 360] call BIS_fnc_relPos;

	_obj_puddle = "BloodPool_01_Large_New_F" createVehicle (_poz_puddle vectorAdd [0,0,0.1]);
...
/* fnc_puddle_trail =
{
	...
	if (_prob_create > 8) then
	{
		...
		_aspect_puddle = ["res\01_lake.paa","res\02_lake.paa","res\03_lake.paa","res\04_lake.paa"] call BIS_fnc_selectRandom;
...
private ["_fart_parts","_part_body","_poz_fart","_dir_move","_array_markers_radioactive","_mrk_temp","_array_temp","_nr_mrk","_carcasa","_core","_tgt_fart","_radius_toxic","_farty_on","_no_puddles","_trail_on","_damage_toxic","_anomalyVSfield","_location_pos"];
...
[_location_pos,_damage_toxic,_radius_toxic] spawn f_fnc_createAIFieldDamageFartyAnomaly;
...
if (_anomalyVSfield) then
{
	...
	for "_i" from 0 to 5 do {_farty_boss setObjectTextureGlobal [_i,"res\02_lake.paa"]};

	_farty_boss setSpeaker "NoVoice";
	_farty_boss disableConversation true;
	_farty_boss setcaptive true;
	_farty_boss addRating -10000;
	_farty_boss setBehaviour "CARELESS";
	_farty_boss enableFatigue false;
	_farty_boss setSkill ["courage", 1];
	_farty_boss setUnitPos "UP";
	RemoveAllItems _farty_boss;
	removeUniform _farty_boss;
	Removevest _farty_boss;
	removeHeadgear _farty_boss;
	removeAllWeapons _farty_boss;
	_farty_boss unassignItem "NVGoggles";
	_farty_boss removeItem "NVGoggles";
	_farty_boss setUnitPos "DOWN";
	...
	{
		_carcasa = "Sign_Sphere200cm_F" createVehicle [0,0,0];
		_carcasa setObjectMaterialglobal [0, "a3\characters_f_bootcamp\common\data\vrarmoremmisive.rvmat"];
		_carcasa setObjectTextureGlobal [0,"res\shell.jpg"];
		_carcasa setdir (random 360);
		_comp_obj_casp pushBack _carcasa;
		_carcasa attachTo [_farty_boss, [0,0,0],_x]

	} forEach _pct_farty;

	_cap_segment = "Sign_Sphere200cm_F" createVehicle [0,0,0];
	_cap_segment setObjectMaterialglobal [0, "a3\characters_f_bootcamp\common\data\vrarmoremmisive.rvmat"];
	_cap_segment setObjectTextureGlobal [0,"res\shell.jpg"];

	_carcasa_coada = "Sign_Sphere200cm_F" createVehicle [0,0,0];
	_carcasa_coada setObjectMaterialglobal [0, "a3\characters_f_bootcamp\common\data\vrarmoremmisive.rvmat"];
	_carcasa_coada setObjectTextureGlobal [0,"res\shell.jpg"];

	_carcasa_coada_1 = "Sign_Sphere200cm_F" createVehicle [0,0,0];
	_carcasa_coada_1 setObjectMaterialglobal [0, "a3\characters_f_bootcamp\common\data\vrarmoremmisive.rvmat"];
	_carcasa_coada_1 setObjectTextureGlobal [0,"res\shell.jpg"];

	_carcasa_coada_2 = "Sign_Sphere200cm_F" createVehicle [0,0,0];
	_carcasa_coada_2 setObjectMaterialglobal [0, "a3\characters_f_bootcamp\common\data\vrarmoremmisive.rvmat"];
	_carcasa_coada_2 setObjectTextureGlobal [0,"res\shell.jpg"];
	...
	_poz_fart = [_location_pos,1,_radius_toxic/2,2,0,0,0] call BIS_fnc_findSafePos;
	...
	sleep 1;
	_farty_loc setpos _location_pos;
	...
	_shell_fist setObjectTextureGlobal [0,"res\01_splash.paa"];
	...
	while {alive _farty_boss} do
	{
		...
		if (count _list_ai_in_range_fart > 0) then
		{
			if (!isNil{_farty_boss getVariable "ascuns"}) then
			{
				...
				_poz_fart = [_location_pos,1,_radius_toxic,2,0,0,0] call BIS_fnc_findSafePos;
				_farty_boss setpos _poz_fart;
				_farty_loc setPos _poz_fart;
				[[_farty_loc],"AL_farty\inout.sqf"] remoteExec ["execVM"];
			...
			while {(_tgt_fart distance _farty_boss < 50) and (alive _tgt_fart)} do
			{
				...
				{
					_splash_img = ["res\01_splash.paa","res\02_splash.paa"] call BIS_fnc_selectRandom;
